    ...
    for dta in resData:
        ...
        if not getCollectionDetail:
            ...
            try:
                ...
                statdata = collectionStat.objects.create(
                    dateLog                 = current,
                    collectionId_id         = getCollection.id,
            ...
            getCollection = NFTCollection.objects.filter(
                collectionName__iexact = dta['collection_name'],
                slug__iexact = urlSlug
            )

            print('getCollection: ', getCollection[0].id)

            try:
                OpenseaRes = requests.get(
                    'https://api.opensea.io/api/v1/collection/{}/stats'.format(
                        urlSlug),
                    headers={
                        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36'}
                )
                ...
                # print('OpenseaData: ', OpenseaData['stats']['one_day_volume'])

                statdata = collectionStat.objects.create(
                    dateLog                 = current,
                    collectionId_id         = getCollection[0].id,
                ...
                print('created Stats: ', statdata)

            except:
                print('not found to update from Opensea')
